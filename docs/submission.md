...
4. Add release notes in `docs/releases/1.0.0.md` (use the intended version for later releases). Commit the notes, tag the reviewed commit `1.0.0`, and push that tag. The release workflow checks it against the manifest, rebuilds and tests, then creates a **draft** GitHub release with those notes.
